document.addEventListener("DOMContentLoaded", function() {
	...
	let divMainWrapper = document.getElementsByClassName("main-wrapper")[0];
	let divUnlistedVideos = document.getElementsByClassName("unlisted-videos")[0];
	let divPublicVideos = document.getElementsByClassName("public-videos")[0];
	...
	let keys = Object.keys(videos).reverse();
	
	for(let i = 0; i < keys.length; i++) {
		let key = keys[i];
		let title = videos[key].title;
		let link = "https://www.youtube.com/watch?v=" + videos[key].link;
		let thumbnail = "./assets/img/thumbnails/" + key + ".jpg";

		let category = divPublicVideos;
		if(key.charAt(0) === "U") {
			category = divUnlistedVideos;
		}
		...
		if(i === keys.length - 1) {
			divMainWrapper.innerHTML += '<div class="bottom-margin"></div>';
		}
	}
